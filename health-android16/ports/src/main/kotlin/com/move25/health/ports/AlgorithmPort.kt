...
data class AlgorithmExecutionPolicy(
    val availableCapabilities: Set<String> = emptySet(),
    val researchAllowed: Boolean = false,
...
data class AlgorithmRequest(
    ...
    val policy: AlgorithmExecutionPolicy = AlgorithmExecutionPolicy(),
)
...
class BuiltInAlgorithmAdapter : AlgorithmPort {
    private val cards = listOf(
        ...
        AlgorithmDefinition("hrv", "1.0.0", ProductLevel.L2_ADVANCED, setOf("validated_rri_or_vendor_hrv"), setOf(ObservationKind.RRI), "quality/1", EvidenceGrade.E1_ENGINEERING, "合格 RRI 的 HRV 趋势", setOf("ECG 诊断", "自主神经疾病诊断"), "Formula/public domain", "docs/algorithm-cards/hrv.md"),
        ...
        definition("temperature", setOf(ObservationKind.SKIN_TEMPERATURE, ObservationKind.BODY_TEMPERATURE), "温度趋势", ProductLevel.L2_ADVANCED),
        definition("workout", setOf(ObservationKind.WORKOUT_DURATION), "运动总量", ProductLevel.L2_ADVANCED),
        definition("gps_route", setOf(ObservationKind.GPS_ROUTE), "轨迹距离与配速", ProductLevel.L2_ADVANCED),
    ...
    override fun execute(request: AlgorithmRequest): Result<DomainError, List<DerivedMetric>> {
        val definition = cards.firstOrNull { it.id == request.definitionId }
            ?: return Result.Err(DomainError("ALGORITHM_NOT_REGISTERED", request.definitionId))
        if (definition.level == ProductLevel.L3_RESEARCH && !request.policy.researchAllowed) {
            return Result.Err(DomainError("RESEARCH_ALGORITHM_BLOCKED", definition.id))
        }
        if (definition.level == ProductLevel.L4_REGULATED && !request.policy.regulatedProduct) {
            return Result.Err(DomainError("REGULATED_ALGORITHM_BLOCKED", definition.id))
        }
        val missingCapabilities = definition.requiredCapabilities - request.policy.availableCapabilities
        if (missingCapabilities.isNotEmpty()) {
            return Result.Err(DomainError("ALGORITHM_CAPABILITY_REQUIRED", missingCapabilities.sorted().joinToString(",")))
        }
        if (request.observations.none { it.kind in definition.requiredInputs }) {
            return Result.Err(DomainError("ALGORITHM_INPUT_REQUIRED", definition.id))
        }
    ...
    private fun definition(
        id: String,
        inputs: Set<ObservationKind>,
        use: String,
        level: ProductLevel = ProductLevel.L1_WELLNESS,
    ) = AlgorithmDefinition(
        id, "1.0.0", level, emptySet(), inputs, "quality/1", EvidenceGrade.E1_ENGINEERING,
